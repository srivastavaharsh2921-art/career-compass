...
(() => {
  ...
  const SIGNALS = [
    ...
    [/\b(database|sql|query|relational|postgres|mysql)\b/i, ["SQL"]],
    [/\b(machine learning|deep learning|pandas|numpy|tensorflow|pytorch)\b/i, ["Python"]],
    [/\b(linux|terminal|shell|docker|ci\/cd)\b/i, ["Bash"]],
    [/\b(kubernetes|infrastructure as code)\b/i, ["YAML", "Bash"]],
    ...
    [/\b(selenium|playwright|cypress)\b/i, ["JavaScript", "Python", "Java"]]
  ];
  ...
  function get(career, node = {}) {
    if (Object.prototype.hasOwnProperty.call(BY_CAREER, career?.id)) {
      return [...BY_CAREER[career.id]];
    }
    const base = BY_CATEGORY[career?.category] || [];
    const text = [career?.label, career?.role, node.title, ...(node.topics || [])].join(" ");
    return fromText(text, base).slice(0, 4);
  }
